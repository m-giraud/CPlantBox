...
class TestOrganism(unittest.TestCase):
    ...
    def hand_example(self):
        ...
        print("num organs ",len(self.human1.getOrgans()))

    def add_nodes(self):#add node at 0
        ...
        self.thumb.addNode(pb.Vector3d(0, -1, 1.6), id= thumb, t=4)
        nodes = np.array((list(map(np.array, self.human1.getNodes()))))
        print("add 0, -1, 1.6\n", nodes)
        self.thumb.addNode(pb.Vector3d(0, -2, 2.5), 4)
        nodes = np.array((list(map(np.array, self.human1.getNodes()))))
        print("add 0, -2, 2.5\n", nodes)
        self.little_finger.addNode(pb.Vector3d(0, 1, 1.6), id= lf, t=3)
