...
anim.min = min_b
anim.max = max_b
anim.res = cell_number
anim.soil_data = True
anim.soil = s
anim.cuts = True  # soil cut 
anim.avi_name = "avi/example_6c_"
anim.start()

for i in range(0, N):

    if rank == 0:  # Root part is not parallel
        rx = r.solve(rs_age + t, -0.2 * trans, 0., sx, True, wilting_point, [])  # xylem_flux.py -trans * sinusoidal(t)
